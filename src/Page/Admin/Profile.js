...
const Profile = () => {
  ...
  // Local save handler
  const updateProfileInHeader = (updatedProfile) => {
  ...
  const handleImageUpload = (e) => {
    ...
    if (file && (file.type === "image/jpeg" || file.type === "image/png")) {
      ...
      reader.onload = () => setProfile(prev => ({
        ...prev,
        profileImage: { ...prev.profileImage, url: reader.result }
      }));
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: '2.25rem', fontWeight: 'bold', color: 'white', letterSpacing: '-0.02em' }}>
            Admin Profile
          </h2>
          <p style={{ color: '#bbf7d0', marginTop: '0.5rem' }}>
            Farmer Assistant Management System
          </p>
        </div>
            ...
            <label htmlFor="profileImageUpload" style={{ position: 'relative', cursor: editMode ? 'pointer' : 'default' }}>
              <div style={{
                ...
                {profile.profileImage?.url ? (
                  <img src={profile.profileImage.url} alt="Profile" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <FaUserCircle style={{ fontSize: '5rem', color: '#86efac' }} />
                )}
                {editMode && (
                  <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', paddingTop: '1.5rem' }}>
              {editMode ? (
                <>
                  ...
                  ><FaSave style={{ marginRight: '0.75rem' }} /> Save Changes</button>
                  <button
                    type="button"
                    onClick={handleEdit}
                    style={{
                  ...
                  ><FaTimes style={{ marginRight: '0.75rem' }} /> Cancel</button>
                </>
              ) : (
                <button
                  type="button"
                ...
                ><FaEdit style={{ marginRight: '0.75rem' }} /> Edit Profile</button>
              )}
            </div>
          </form>
        </div>
